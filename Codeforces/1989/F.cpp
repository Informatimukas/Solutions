...
struct UnionSet {
  vector<int> par, siz;
  ll value;
  UnionSet(int n): par(n), siz(n, 1), value(0) {
  ...
  void unionSet(int a, int b) {
    ...
    if (siz[a] < siz[b])
      swap(a, b);
    if (siz[a] > 1)
      value -= static_cast<ll>(siz[a]) * siz[a];
    if (siz[b] > 1)
      value -= static_cast<ll>(siz[b]) * siz[b];
    siz[a] += siz[b];
    par[b] = a;
    value += static_cast<ll>(siz[a]) * siz[a];
  }
...
int main()
{
    ...
    auto got = offline_incremental_scc(edges, N);
    vector events(q + 1, vector<array<int, 2>>());
    for (int i = 0; i < got.size(); i++)
      events[got[i]].push_back(edges[i]);
    for (int i = 0; i < q; i++) {
